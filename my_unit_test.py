...
def test_import():
    for index, batch in enumerate(IMPORT_BATCHES[:4]):
...
def test_nodes():
    ...
    deep_sort_children(EXPECTED_TREES[0])
    if response != EXPECTED_TREES[0]:
        print_diff(EXPECTED_TREES[0], response)
...
def test_sales():
    status, _ = request("/imports", method="POST", data=IMPORT_BATCHES[4])
    params = urllib.parse.urlencode({
        "date": "2022-02-04T17:00:00.000Z"
    })
    status, response = request(f"/sales?{params}", json_response=True)
    assert status == 200, f"Expected HTTP status code 200, got {status}"
    assert response == EXPECTED_TREES[1], "Sales wrong!"
...
def test_stats():
    ...
    assert status == 200, f"Expected HTTP status code 200, got {status}"
    assert response == EXPECTED_TREES[2], "Wrong stats response"
